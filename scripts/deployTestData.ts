...
async function main() {
  ...
  const swobAddress = "0x1F5A1dA9995149E9D1697c6d0E2ca4a312A496fE";
  ...
  tx = await orderBook.setTokenIdInfos([tokenId], [{tick, minQuantity: 1}]);
  await tx.wait();
  console.log("orderBook.setTokenIdInfos");

  tx = await orderBook.connect(owner).limitOrders([
    {
      side: OrderSide.Buy,
      tokenId,
      price,
