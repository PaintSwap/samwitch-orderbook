...
*Sam Witch (PaintSwap &amp; Estfor Kingdom)0xDoubleSharp*
...
The minimum amount that can be added to the order book for a specific token ID, to keep the order book healthy
...
| Name | Type | Description |
|---|---|---|
| _tokenId | uint256 | The token ID to get the minimum quantity for |
...
Get the tick size for a specific token ID
...
| Name | Type | Description |
|---|---|---|
| _tokenId | uint256 | The token ID to get the tick size for |
...
```solidity
function nextOrderId() external view returns (uint40)
```
...
```solidity
function tokensClaimable(uint40[] _orderIds, bool takeAwayFees) external view returns (uint256 amount_)
```
